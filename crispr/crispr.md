...
![Pic](./pics/fig3.jpg)
*<sub>Figure 3. The dCas9-protein complex can interfere with transcription. Source: https://en.wikipedia.org/wiki/File:CRISPR_Sterics.pdf.</sub>*
...
![Pic](./pics/fig4.png)
*<sub>Figure 4. Construction of the plasmid containing the sgRNA. Phosphorylated primers F (with crRNA sequence; green) and R are used in an iPCR to multiply the linear plasmid (A). The iPCR mixture contains linear PCR products (red and blue) and the template (black) (C). The mixture is then treated with DpnI to remove the template (C). The linear plasmids are circularized using T4 DNA Ligase (D). Source: own work. Licence: CC-BY.</sub>*
